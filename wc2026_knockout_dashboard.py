"""
World Cup 2026 Knockout Stage Dashboard

Trains the model fresh on startup, then on each run:
  - fetches the latest results CSV from RAW_URL
  - rebuilds features for completed and upcoming knockout matches
  - predicts upcoming fixtures
  - backtests WC 2026 knockout matches that now have scores

There is no draw outcome here, since knockout matches go to extra time
and penalties until a winner is found.

Requires wc2026.py and wc2026_knockout.py in the same folder.
"""
...
def predict_result(df):
    return (df["prob_home_win"] >= df["prob_away_win"]).map({True: "home_win", False: "away_win"})
...
def label_result(row, col):
    val = row[col]
    return row["home_team"] if val == "home_win" else row["away_team"]
...
st.set_page_config(page_title="WC 2026 Knockout Predictions", page_icon="⚽", layout="wide")
st.title("⚽ FIFA World Cup 2026 — Knockout Stage Predictions")
st.caption("No draw outcome here. Knockout matches go to extra time and penalties until a winner is found.")

@st.cache_resource(show_spinner="Training knockout model on latest data...")
def load_model_and_data():
    df_hist, df_upcoming = wc.load_data(wc.RAW_URL)
    df_features  = wck.build_feature_matrix_knockout(df_hist)
    df_predict   = wc.build_upcoming_features(df_upcoming, df_hist)
    last_played  = df_hist["date"].max()
    X_train, X_test, y_train, y_test = wck.split_data(df_features)
    clf_pipeline, _, le, clf_classes, _, _, _ = \
        wck.train_calibrated_classifier(X_train, y_train, X_test, y_test)
...
with st.sidebar:
    ...
    st.caption(f"Upcoming knockout fixtures: **{len(df_results)}**")
    st.caption(f"Knockout matches played: **{len(df_backtest)}**")
...
with tab_upcoming:
    st.subheader("Upcoming Knockout Fixtures")
    st.caption("Calibrated win probabilities from a binary XGBoost model. Probabilities always sum to 100%.")

    if df_results.empty:
        st.info("No upcoming knockout fixtures without scores.")
    else:
        ...
        for _, row in df_show.iterrows():
            with st.container():
                ...
                pred_label = row["home_team"] if row["pred_result"] == "home_win" else row["away_team"]

                st.caption(f"📅 {row['date']}  |  🏆 {row['tournament']}  |  Predicted to advance: **{pred_label}**")
                st.divider()
...
with tab_backtest:
    st.subheader("Played Knockout Matches")
    st.caption("Model predictions vs actual results for knockout matches already played.")

    if df_backtest.empty:
        st.info("No knockout matches with settled scores yet.")
    else:
        ...
        st.dataframe(
            df_bt_show[display_cols].style
                ...
                .format({"prob_home_win": "{}%", "prob_away_win": "{}%"}),
...
with tab_summary:
    ...
    if df_backtest.empty:
        st.info("Summary metrics appear once knockout matches have been played.")
    else:
